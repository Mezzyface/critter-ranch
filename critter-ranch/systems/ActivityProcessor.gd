...
# Changed from static functions to regular functions since it's now an autoload

func can_afford_activity(activity: ActivityType.Activity) -> bool:
...
func process_activity(creature_data: Dictionary, activity: ActivityType.Activity) -> Dictionary:
	var key = ActivityType.get_key(activity)
	var cost = GameConstants.ACTIVITY_COSTS.get(key, 0)
